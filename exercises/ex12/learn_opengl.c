...
main(int argc, char* argv[])
{
	...
	while (!glfwWindowShouldClose(window.handle))
	{
		...
		glm_rotate(trans1, (float)glfwGetTime(), (vec3){ 0.0f, 0.0f, 1.0f });

		// First activate the shader program.
		shader_bind(shader_program);
		shader_uniform_mat4(shader_program, "transform", trans1);
		...
		glm_scale(trans2, (vec3){ sin(glfwGetTime()), cos(glfwGetTime()), 0.0f });

		// First activate the shader program.
		shader_bind(shader_program);
		shader_uniform_mat4(shader_program, "transform", trans2);
